...
static void mavlink_test_avoidcontrolflow(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_AvoidControlFlow >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_avoidcontrolflow_t packet_in = {
        963497464,963497672,963497880,963498088,963498296,963498504,{ 185.0, 186.0, 187.0, 188.0 }
    };
    mavlink_avoidcontrolflow_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.suggest_move_x = packet_in.suggest_move_x;
        packet1.suggest_move_y = packet_in.suggest_move_y;
        packet1.suggest_move_z = packet_in.suggest_move_z;
        packet1.prefer_target_x = packet_in.prefer_target_x;
        packet1.prefer_target_y = packet_in.prefer_target_y;
        packet1.prefer_target_z = packet_in.prefer_target_z;
        
        mav_array_memcpy(packet1.reservered_param, packet_in.reservered_param, sizeof(float)*4);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_AvoidControlFlow_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_AvoidControlFlow_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_avoidcontrolflow_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_avoidcontrolflow_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_avoidcontrolflow_pack(system_id, component_id, &msg , packet1.suggest_move_x , packet1.suggest_move_y , packet1.suggest_move_z , packet1.prefer_target_x , packet1.prefer_target_y , packet1.prefer_target_z , packet1.reservered_param );
    mavlink_msg_avoidcontrolflow_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_avoidcontrolflow_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.suggest_move_x , packet1.suggest_move_y , packet1.suggest_move_z , packet1.prefer_target_x , packet1.prefer_target_y , packet1.prefer_target_z , packet1.reservered_param );
    mavlink_msg_avoidcontrolflow_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_avoidcontrolflow_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_avoidcontrolflow_send(MAVLINK_COMM_1 , packet1.suggest_move_x , packet1.suggest_move_y , packet1.suggest_move_z , packet1.prefer_target_x , packet1.prefer_target_y , packet1.prefer_target_z , packet1.reservered_param );
    mavlink_msg_avoidcontrolflow_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_ardupilotmega(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_avoidcontrolflow(system_id, component_id, last_msg);
}
